...
func _ready() -> void:
	...
	_spawn_torches()
	lbl_wave.label_settings = lbl_wave.label_settings.duplicate()
	lbl_wave.label_settings.font_size = 18
	lbl_time.label_settings = lbl_time.label_settings.duplicate()
	lbl_time.label_settings.font_size = 15
...
func on_wave_started(w: int) -> void:
	var title: String = RunCampaign.NAMES[wave_controller.stage]
	if wave_controller.stage == 2:
		title = "Obsidian Obelisk\nThe Darkest Darkness"
	lbl_wave.text = "%s  •  Part %d / 3\n%s" % [title, wave_controller.stage + 1, "Arrival" if w == 0 else "Wave %d / 10" % w]

func on_wave_time(_t: float) -> void:
	var elapsed := GameState.run_elapsed_seconds
	var area_start := float(wave_controller.stage) * RunCampaign.AREA_SECONDS
	var remaining := float(wave_controller.stage + 1) * RunCampaign.AREA_SECONDS - elapsed
	var objective := "Defeat the boss before time runs out"
	if wave_controller.wave < 10:
		objective = "Next wave in " + _clock_text(_t)
	if elapsed < area_start:
		objective = "Waves begin in " + _clock_text(area_start - elapsed)
		remaining = RunCampaign.AREA_SECONDS
	elif wave_controller.stage_boss_killed:
		objective = "Enter the portal before the area ends" if wave_controller.stage < 2 else "All bosses defeated • Survive until 15:00"
	lbl_time.text = "Run %s / 15:00  •  Area %s\n%s" % [_clock_text(floor(elapsed)), _clock_text(remaining), objective]
...
func _on_player_died() -> void:
	end_run(false, wave_controller.completed_waves)

func end_run(won: bool, waves_completed: int) -> void:
	# Prevent double-end
	if game_over_panel.visible:
		return
	if won and (not GameState.run_boss_killed or wave_controller.bosses_defeated != 3):
		won = false
	wave_controller.finished = true
	wave_controller.spawning = false
	wave_controller.spawn_timer.stop()
	get_tree().paused = true
	...
	$UI/GameOverPanel/Margin/VBox/Result.text = result_text + "  —  Waves completed: %d / 30" % waves_completed + "\n" + wave_controller.failure_reason
...
func transition_to_area(area: int) -> void:
	# Keep the live Player and its children: health, status durations, pets,
	# weapon controllers and every run upgrade survive the portal.
	for container_name in ["EnemyContainer", "PickupContainer"]:
		for child in get_node(container_name).get_children():
			child.queue_free()
	for child in get_children():
		if child is Node2D and child != player and child.name not in ["EnemyContainer", "PickupContainer", "ArenaWalls"]:
			if child.is_in_group("pets"):
				child.global_position = wave_controller.arena_rect.get_center() + Vector2(60, 0)
				child.reset_physics_interpolation()
			else:
				child.queue_free()
	current_charge_shrine = null
	var floor_rect := $ArenaFloor as TextureRect
	floor_rect.texture = load("res://assets/campaign/volcanic-rock.png" if area == 1 else "res://assets/campaign/starfield.png") as Texture2D
	# Mirrored repeat makes both generated texture boundaries continuous.
	var shader := Shader.new()
	shader.code = "shader_type canvas_item; void fragment() { vec2 p = abs(fract(UV * 2.0) * 2.0 - 1.0); COLOR = texture(TEXTURE, p); }"
	var material := ShaderMaterial.new()
	material.shader = shader
	floor_rect.material = material
	$Background.color = Color("241011") if area == 1 else Color("070611")
	for wall in $ArenaWalls.get_children():
		wall.get_node("Visual").color = Color("381b1c") if area == 1 else Color("171328")
	player.global_position = wave_controller.arena_rect.get_center()
	player.velocity = Vector2.ZERO
	player._has_click_target = false
	player.reset_physics_interpolation()
	player.get_node("Camera2D").reset_smoothing()
	player.get_node("Camera2D").force_update_scroll()
	var minimap := $UI.get_node_or_null("Minimap")
	if minimap:
		minimap.initialize(wave_controller.arena_rect, player)
	_spawn_initial_gifts()
	_spawn_initial_chests()
	_spawn_initial_powerups()
	_spawn_initial_crystal()
